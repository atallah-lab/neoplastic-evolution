...
#         print(gene_hits)
    ...
    if (gene_hits > 0) {
        ...
        r = r*gainp^tsg_hits # TSG insertion doubles cell division rate
    }
...
maybeTranspose <- function(node,tnum) {
    ...
    if (sample(x=c(0,1),1,prob=c(1-node$tp, node$tp))) {
        ...
        r_tmp <- rank_clone(node$r, exann, simout[[2]], simout[[3]],1.2,0.8)
...
intp <- 1       # Initial probability of transposition
